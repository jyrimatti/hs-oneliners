...
case $FILE_OR_PKGS in
	...
	*.cabal)
		...
		nix-shell -I n1803=channel:nixos-18.03 -I n1809=channel:nixos-18.09 -p $devtools $devtools2 $devtools3 $devtools4 $devtools5 $devtools6 --command "export ORIG_PATH=\$PATH; nix-shell --pure --keep ORIG_PATH $ARGS --argstr compiler ghcjs --command 'PATH=\$PATH:\$ORIG_PATH return' /tmp/hs.nix"
		;;
	# Create shell environment based no a given nix file (e.g. ./default.nix)
	*.nix)
		# orig_path trick because nix-shell orders path in an odd way: https://github.com/NixOS/nix/issues/1671
		nix-shell -I n1803=channel:nixos-18.03 -I n1809=channel:nixos-18.09 -p $devtools $devtools2 $devtools3 $devtools4 $devtools5 $devtools6 --command "export ORIG_PATH=\$PATH; nix-shell --pure --keep ORIG_PATH $ARGS --argstr compiler ghcjs --command 'PATH=\$PATH:\$ORIG_PATH return' $FILE_OR_PKGS"
		;;
	# Assume a list of haskell packages was given, and create an environment with those packages
	*)
		nix-shell -I n1803=channel:nixos-18.03 -I n1809=channel:nixos-18.09 $ARGS -p "haskell.packages.ghcjs.ghcWithPackages (p: with p; [$devtools $devtools2 $devtools3 $devtools4 $devtools5 $devtools6 $FILE_OR_PKGS])"
		;;
esac
